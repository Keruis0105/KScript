...
namespace Backend::String {
    ...
    inline __attribute__((always_inline, pure)) size_t strlen_scalar(const CharT* __restrict__ str) noexcept {
        ...
        constexpr MaskT mask01 = strlen_mask<CharT>::kMask01;
        constexpr MaskT mask80 = strlen_mask<CharT>::kMask80;
        constexpr size_t num_chars = sizeof(MaskT)/sizeof(CharT);

        auto* lp = align_ptr<MaskT>(str);
        MaskT va = *lp;
        MaskT vb = ~*lp & mask80;
        lp++;
